...
-(id)initWithPublicKeyFilePath:(NSString*)publicKeyPath
{
    self = [super init];
    if(![self configurePublicKeyWithFilePath:publicKeyPath]) return nil;
    return self;
}
...
- (BOOL)configurePublicKey
{
    NSString *publicKeyPath = [[NSBundle mainBundle]
                               pathForResource:@"public_key"
                               ofType:@"der"];
    
    if (publicKeyPath == nil) {
        [NSException raise:@"PublicKeyNotFound" format:@"Cannot find pub.der"];
        return NO;
    }
    
    return [self configurePublicKeyWithFilePath:publicKeyPath];
}

- (BOOL)configurePublicKeyWithFilePath:(NSString*)publicKeyPath
{
    NSData *publicKeyFileContent = [NSData dataWithContentsOfFile:publicKeyPath];
    if (publicKeyFileContent == nil) {
        [NSException raise:@"PublicKeyNotFound" format:@"Cannot read from pub.der"];
        return NO;
    }
